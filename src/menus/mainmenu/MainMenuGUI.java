...
public MainMenuGUI() {
	...
	btnHeal.addActionListener(e -> {
		if (ht.getPokeSlots().isEmpty()) throw new ExitFunctionException("You cannot heal your pokemon if you don't have any!", errorLabel);
		ht.fullyHealPokemon();
	});

	btnStartBattle.addActionListener(e -> {
		if (ht.getPokeSlots().isEmpty()) throw new ExitFunctionException("You cannot start a battle with 0 pokemon!", errorLabel);
